...
> ℹ️ **Storage(파일 저장소)는 만들 필요 없습니다.**
> 음원(MR·가이드)은 **구글 드라이브 공유 링크**로 연동하는 방식이라 Firebase Storage도, 요금제 업그레이드(카드 등록)도 필요 없어요.

✅ 여기까지 하면 Firebase 준비 끝!

### 음원 자료실 사용법 (구글 드라이브 연동)
1. 본인 구글 드라이브에 MR·가이드 음원 파일을 올립니다.
2. 파일 우클릭 → **공유** → **‘링크가 있는 모든 사용자’ + 뷰어** 로 설정 → **링크 복사**
3. 홈페이지 → 음원 자료실 → 작품 폴더 선택 → **곡명 + 링크** 붙여넣고 「음원 추가」
4. 단원은 「다운로드」를 누르면 드라이브에서 내려받습니다.
   - (드라이브 파일이 ‘링크 공유’로 열려 있어야 단원이 받을 수 있어요)
...
- **음원 다운로드가 안 돼요** → 구글 드라이브 파일이 ‘링크가 있는 모든 사용자(뷰어)’로 공유돼 있는지 확인하세요.
```
