      ...
      {editing ? (
        <div className="card space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={save} disabled={busy} className="flex-1 rounded-xl bg-[#6f7f8f] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#5e6e7d] disabled:opacity-40">{busy ? "저장 중…" : "저장"}</button>
      ...
      {!editing && (
        <section className="card">
                    ...
                    <div key={c.id} className="py-3">
                      ...
                      {replyTo === c.id && (
                        <div className="mt-2 flex gap-2 border-l-2 border-slate-100 pl-3 sm:ml-10">
                          ...
                          />
                          <button onClick={() => submitReply(c.id)} disabled={commentBusy || !replyText.trim()} className="rounded-lg bg-[#6f7f8f] px-3 py-1.5 text-xs font-bold text-white transition hover:bg-[#5e6e7d] disabled:opacity-40">
                            등록
                          </button>
                        </div>
                      )}
          ...
          <div className="mt-3 flex gap-2">
            ...
            />
            <button onClick={addComment} disabled={commentBusy || !commentText.trim()} className="rounded-xl bg-[#6f7f8f] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#5e6e7d] disabled:opacity-40">
              등록
            </button>
          </div>
        </section>
